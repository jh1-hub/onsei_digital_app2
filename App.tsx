...
const AudioGenerators: Record<string, (ctx: AudioContext) => AudioBuffer> = {
    simple: (ctx) => {
...
const processAudioData = (rawBuffer: AudioBuffer, targetRate: number, bitDepth: number) => {
...
const WaveformVisualizer = ({ 
  ...
  isInspectorOpen: boolean;
    ...
    const getX = (index: number, width: number) => ((index - startSample) / windowSizeSamples) * width;
    const getY = (val: number, height: number) => (1 - (val + 1) / 2) * height;
    const getIndexFromX = (x: number, width: number) => {
        const ratio = x / width;
        return Math.floor(ratio * windowSizeSamples) + startSample;
    };

    const updateCursor = (e: React.MouseEvent | React.Touch) => {
    ...
    }, [originalData, processedData, targetRate, cursorIndex, isInspectorOpen]);
    ...
    return (
        <div 
            ...
            onMouseDown={(e) => { if(isInspectorOpen) { setIsDragging(true); updateCursor(e); } }}
            onMouseMove={(e) => { if(isDragging && isInspectorOpen) updateCursor(e); }}
            onMouseUp={() => setIsDragging(false)}
            onMouseLeave={() => setIsDragging(false)}
            onTouchStart={(e) => { if(isInspectorOpen) { setIsDragging(true); updateCursor(e.touches[0]); } }}
            onTouchMove={(e) => { if(isDragging && isInspectorOpen) updateCursor(e.touches[0]); }}
...
export default function App() {
    ...
    const startRecording = async () => {
        ...
        try {
            ...
            // Refresh device list to get labels if they were empty
            updateDevices();
    ...
    const generateTone = (type: string) => {
        const ctx = initAudio();
    ...
    const loadBuffer = (buffer: AudioBuffer) => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-8 pb-16 font-sans text-slate-800">
            <header className="mb-8 text-center">
                <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-2">
                    音のデジタル化実験室 <span className="text-cyan-600 text-base align-middle ml-2 border border-cyan-200 bg-cyan-50 px-3 py-1 rounded-full font-mono">SoundBit</span>
                </h1>
                    ...
                    <div className="flex flex-wrap gap-2 justify-center">
                        <div className="flex items-center gap-1">
                            {!isRecording ? (
                                ...
                                <button onClick={stopRecording} className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-rose-500 border border-rose-200 rounded-lg font-bold recording-active">
                                    <Square size={20} /> 停止
                                </button>
                            )}
                            ...
                            {devices.length > 1 && (
                                <div className="relative group">
                                    <select 
                                        ...
                                        className="appearance-none text-[10px] bg-white border border-slate-200 rounded-lg pl-2 pr-6 py-2.5 text-slate-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 max-w-[100px] truncate cursor-pointer hover:bg-slate-50 transition-colors"
                ...
                <section className="flex flex-col items-center justify-center py-6 mb-8 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                    {!isPlaying ? (
                        <button onClick={playProcessedAudio} className="group relative inline-flex items-center gap-3 px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white text-lg font-bold rounded-full shadow-lg shadow-cyan-200 transition-all hover:scale-105 active:scale-95">
                            <Play size={20} fill="currentColor" /> 変換後の音を再生
                        </button>
                    ) : (
                    ...
                    <AnimatePresence>
                      {showInspector && (
                          <motion.div 
                            ...
                            className="mt-4"
                          >
                               <p className="text-center text-xs text-rose-500 mb-2">
            ...
            <footer className="mt-12 text-center text-slate-400 text-xs">
                <p>Information I Study App - SoundBit</p>
            </footer>
        </div>
    );
}
